...
public class DialogHelper extends DialogFragment implements View.OnClickListener {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        if (mDialogType == R.integer.MSG_TYPE_NOTIFICATION) {
            ...
            if (btnYes.getVisibility() == View.VISIBLE) btnYes.setVisibility(View.GONE);
            if (btnNo.getVisibility() == View.VISIBLE) btnNo.setVisibility(View.GONE);
            if (btnOk.getVisibility() != View.VISIBLE) btnYes.setVisibility(View.VISIBLE);
        }
        if (mDialogType == R.integer.MSG_TYPE_ACTION) {
            //disable ok and only display yes, no
            if (btnOk.getVisibility() == View.VISIBLE) btnOk.setVisibility(View.GONE);
            if (btnYes.getVisibility() != View.VISIBLE) btnYes.setVisibility(View.VISIBLE);
            if (btnNo.getVisibility() != View.VISIBLE) btnNo.setVisibility(View.VISIBLE);
        }
        switch (mSeverity) {
    ...
    public void onClick(View v) {
        if (v == btnYes) {
            //do
            communicator.onDialogMessage(R.integer.MSG_YES);
            dismiss();
        }
        if (v == btnNo) {
            //do
            communicator.onDialogMessage(R.integer.MSG_NO);
            dismiss();
        }
        if (v == btnOk) {
            //do
